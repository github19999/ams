...
# ==============================================================================
#
# 【优化1 - 模块4 交互顺序调整为「端口 → 域名 → UUID/密码」】
#   v2版: 交互顺序与协议逐一对应，已支持三协议独立配置
#   v3版: 顺序进一步明确为每个协议完整收集：端口 → 域名/IP → UUID（或SS密码）
#   交互示例:
#     ── anytls 配置 ──
#     anytls 端口 [默认: 48790]:
#     anytls 服务器域名或 IP:
...
#     vless UUID [默认: 随机生成]:
#     ── shadowsocks 配置 ──
#     shadowsocks 端口 [默认: 46790]:
#     shadowsocks 服务器域名或 IP:
#     SS 密码 [默认: 随机生成]:
#     证书目录路径 [默认: /etc/ssl/private/]:
#
# 【优化2 - 证书路径彻底消除与模块2的重复感】
#   问题: 模块2 询问了证书存放路径，模块4 又询问证书目录，用户感觉重复
#   v2版: 若模块2已执行则自动同步路径，但仍弹出确认提示，仍有一步交互
#   v3版: 若模块2已执行（M2_CERT_PATH 有值），模块4 完全静默同步，
#         零交互直接使用，不弹任何提示；仅在模块2未执行时才询问路径
#   说明: 模块2 负责「申请和安装证书」，模块4 负责「引用证书路径生成配置」
#         两者各司其职——模块4 不会再次申请证书，只读取路径写入 config.json
#
# 【优化3 - UUID/SS密码默认值改为显示「随机生成」占位提示】
#   v2版: 提示行直接展示随机生成的完整值，行很长，视觉上信息量大
#   v3版: 提示行显示「随机生成」文字占位；用户直接回车则静默使用随机值，
#         若需自定义直接输入覆盖即可，视觉更简洁
#
# ==============================================================================
...
module_4_config_singbox() {
  ...
  echo ""
  echo -e "${CYAN}── anytls 配置 ─────────────────────────────${RESET}"
  ask "anytls 端口" "48790" M4_ANYTLS_PORT
  ask "anytls 服务器域名或 IP" "" M4_ANYTLS_DOMAIN
  local _uuid1; _uuid1=$(gen_uuid)
  ask "anytls UUID" "随机生成" _anytls_uuid_input
  [[ "$_anytls_uuid_input" == "随机生成" ]] && M4_ANYTLS_UUID="$_uuid1" || M4_ANYTLS_UUID="$_anytls_uuid_input"
  ...
  ask "vless UUID" "随机生成" _vless_uuid_input
  [[ "$_vless_uuid_input" == "随机生成" ]] && M4_VLESS_UUID="$_uuid2" || M4_VLESS_UUID="$_vless_uuid_input"

  echo ""
  echo -e "${CYAN}── shadowsocks 配置 ────────────────────────${RESET}"
  ask "shadowsocks 端口" "46790" M4_SS_PORT
  ask "shadowsocks 服务器域名或 IP" "" M4_SS_DOMAIN
  local _ss_default; _ss_default=$(gen_ss_pass)
  ask "SS 密码" "随机生成" _ss_pass_input
  [[ "$_ss_pass_input" == "随机生成" ]] && M4_SS_PASS="$_ss_default" || M4_SS_PASS="$_ss_pass_input"

  echo ""
  echo -e "${CYAN}── 证书配置 ────────────────────────────────${RESET}"
  # 若模块2已执行，完全静默同步证书路径，零交互
...
module_6_links() {
  ...
  echo ""
  echo -e "${CYAN}── anytls 参数 ─────────────────────────────${RESET}"
  [[ $M4_ANYTLS_PORT -eq 0 ]]      && ask "anytls 端口"        "48790" M4_ANYTLS_PORT
  [[ -z "$M4_ANYTLS_DOMAIN" ]]     && ask "anytls 服务器域名/IP" ""    M4_ANYTLS_DOMAIN
  [[ -z "$M4_ANYTLS_UUID" ]]       && ask "anytls UUID"         ""    M4_ANYTLS_UUID

  echo ""
  echo -e "${CYAN}── vless 参数 ──────────────────────────────${RESET}"
  [[ $M4_VLESS_PORT -eq 0 ]]       && ask "vless 端口"          "47790" M4_VLESS_PORT
  [[ -z "$M4_VLESS_DOMAIN" ]]      && ask "vless 服务器域名/IP"  ""    M4_VLESS_DOMAIN
  [[ -z "$M4_VLESS_UUID" ]]        && ask "vless UUID"           ""    M4_VLESS_UUID

  echo ""
  echo -e "${CYAN}── shadowsocks 参数 ────────────────────────${RESET}"
  [[ $M4_SS_PORT -eq 0 ]]          && ask "shadowsocks 端口"     "46790" M4_SS_PORT
